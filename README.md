...
**核心技术：** Canvas 2D 像素级水波模拟 + MediaPipe Hands

### 功能说明
基于物理模拟的实时水波纹特效。摄像头画面通过水波折射算法变形，指尖划过画面时产生逼真的涟漪。

- 💧 **水波物理**：双缓冲波方程模拟，带阻尼衰减，指尖轨迹产生连续流体流动
- 🌈 **折射渲染**：像素级位移映射 + 梯度着色，模拟光线折射/反射
- 👆 **指尖涟漪**：仅指尖触发（5 个关键点），手指位置插值保证连续波纹
- 🖱️ **触控支持**：鼠标拖拽 / 触摸屏也可制造波纹
- 🦴 **手部骨架**：半透明青色手部骨架叠加显示

### 使用方式
1. 浏览器打开 `index.html`
2. 允许摄像头权限
3. **手指在镜头前移动**，画面产生水波折射效果
4. 也可用**鼠标拖拽**或**触屏滑动**制造涟漪

### 技术亮点
- 纯 Canvas 2D 实现，无需 WebGL，兼容性极好
- 640×480 分辨率下单帧完成波模拟 + 像素位移 + 渲染
- 双缓冲交替计算，damping=0.99 实现持久波纹衰减
